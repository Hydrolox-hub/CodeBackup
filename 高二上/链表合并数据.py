# -*- coding: utf-8 -*-
"""
Created on Tue Aug 31 14:29:39 2021
...
tmp=randint(95,100)
data_a.append([tmp,head_a]) #添加第一个节点
head_a=0
for i in range(1,5):
    tmp=data_a[i-1][0]-randint(1,5)
    data_a.append([tmp,data_a[i-1][1]])   #链表一 尾部添加新节点[tmp,-1]
    data_a[i-1][1]=i
print(" 链表结构的原始数据序列一：")
print(data_a)
...
for i in range(1,5):
    ...
    data_b.append([tmp,data_b[i-1][1]])   #链表二 尾部添加新节点[tmp,-1]
    data_b[i-1][1]=i
print(" 链表结构的原始数据序列二： ")
print(data_b)
...
while(k_a!=-1 and k_b!=-1):
    ...
    if data_a[k_a][0]>=data_b[k_b][0]:
        ...
        if k_a==head_a:  # 在链表 data_a 的头部插入结点
            ...
            head_a=len(data_a)-1 #新插入的数据成为头节点（最小）
